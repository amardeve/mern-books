                ...
                <div className='flex justify-between items-center text-base gap-8'>
                    ...
                    <ul className='md:flex space-x-3 hidden bg-rose-200/50 px-3 py-1.5 rounded-full'>
                        {navItems.map(({ link, path }) => (
                            <Link key={path} to={path} className={`block text-base uppercase cursor-pointer hover:bg-white items-center py-1.5 px-3 rounded-full duration-300 font-semibold ${location.pathname === path ? 'bg-gray-300 text-blue-700' : 'text-rose-800'}`}>
                                {link}
                            </Link>
                        ))}
